...
using keymap_t = std::map<uint64_t, size_t>;
...
std::set<size_t> clean_records;
std::atomic<uint64_t> max_ID;

sharedMtx rwmutex;
std::vector<uint> thread_ids;
std::mutex thread_run_mutex;
...
void dump(void)
{
    ...
    fwrite(data.data(), sizeof(record_t), s, fdata);
...
void select(uint64_t key, record_t * buf)
{
...
void update(uint64_t key, const record_t & record)
{
    size_t pos = keysmap[key];
    record_t & rec = data[pos];
    auto arange = athrmap.equal_range(std::string(rec.author));
...
void insert(uint64_t key, const record_t & record)
{
    size_t pos;
...
void thread_fun(int tid, int sockid)
{
    int chk = 1;
...
std::vector<record_t> generate(int count, int art, int aut, int jou, int yearmin, int yearmax, int numbmin, int numbmax)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        record_t rec(i + 1, gaut.c_str(), gy, gn, gjou.c_str(), gart.c_str());
        res.push_back(rec);
    }
    return res;
}
...
void ggen (int cnt)
{
    std::vector<record_t> gen = generate(cnt, 200000, 10000, 1000, 1900, 2000, 1, 1000);
