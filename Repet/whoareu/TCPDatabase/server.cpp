...
const int query_pos_type = 1;
const int query_pos_author = 1;
const int query_pos_year1 = 2;
const int query_pos_year2 = 3;
const int query_pos_number1 = 4;
const int query_pos_number2 = 5;
...
const char select_regex[] = " *select *(?>author *= *\\[(.+?)\\] *)?(?:year *= *\\[ *(\\d+) *, *(\\d+) *\\] *)?(?:number *= *\\[ *(\\d+) *, *(\\d+) *\\] *)?";
...
response_t execute_query(char * query)
{
	...
	boost::regex pattern(select_regex);
    boost::cmatch mres;
    if (!boost::regex_match(query, mres, pattern))
    	return std::make_pair<record_t *, int32_t> (nullptr, -1);
    std::vector<keysearch_t> sets;
    if (mres[query_pos_author].matched)
    {
    	sets.push_back(searcha((std::string)mres[query_pos_author]));
    }
    if (mres[query_pos_year1].matched)
    {
    	int left, right;
    	try
    	{
    		left = std::stoi((std::string)mres[query_pos_year1]);
    		right = std::stoi((std::string)mres[query_pos_year2]);
    	}
    ...
    if (mres[query_pos_number1].matched)
    {
    	int left, right;
    	try
    	{
    		left = std::stoi((std::string)mres[query_pos_number1]);
    		right = std::stoi((std::string)mres[query_pos_number2]);
    	}
    	catch (const std::out_of_range & ex)
    	{
    ...
    if (sets.size() == 0)
    {
    	...
    	return std::make_pair<record_t *, int32_t> (std::move(result), keysmap.size());
    }
    if (sets.size() == 1)
    {
    	int sz = sets[0].size();
...
void thread_fun(int tid, int sockid)   //  poluchaet  ID potoka (1 / 16) i  ID soceta s kotorm budet rabotat
{
    ...
    int chk = 1;
    sendall(sockid, (char *)&chk, sizeof(int), MSG_NOSIGNAL);
    recvall(sockid, (char *)&chk, sizeof(int), MSG_NOSIGNAL);
    char com[max_com_len];
    
    while (true)
    {
    	int32_t len;
        recvall(sockid, (char *)&len, sizeof(int), MSG_NOSIGNAL); //without null-terminator at the end of command
        if (len == 0)
        	break;
    	if (chk)
    		len = swapbytes(len);
    	if (len > max_com_len)
    		len = max_com_len;
    	recvall(sockid, com, len, MSG_NOSIGNAL);
    	com[len] = '\0';
    	response_t response = execute_query(com);
    	sendall(sockid, (char *)&response.second, sizeof(int), MSG_NOSIGNAL);
    	if (response.second >= 0)
    		sendall(sockid, (char *)response.first, response.second * sizeof(record_t), MSG_NOSIGNAL);
    	free(response.first);
...
int main(int argc, char const *argv[])
{
	...
	/*
	std::vector<record_t> gen = generate(300000, 200000, 10000, 1000, 1900, 2000, 1, 1000);
	...
	*/
	...
	for (auto it = athrmap.begin(); it != athrmap.end(); ++it)
	{
		/* code */
	}
