...
BInt            OpPlus     (BInt & a, BInt & b)
{
	...
	IsZero(res);
	if (carry) res.push_back((sgn) ? (uint) -1 : 1);
	return res;
}
void            OpPlusEqu  (BInt & a, BInt & b)
{
	...
	IsZero(a);
	if (carry) a.push_back((sgn) ? (uint) -1 : 1);
}
...
int main()
{
	...
	for (int i = 1; i < n; ++i)
	{
		arr[i].resize((m - 1) * (i + 1) + 1);
		for (int j = 0; j < (m - 1) * (i + 1) + 1; ++j)
		{
			if (j > (m - 1) * (n / 2)) break;
			IsZero(arr[i][j]);
			for (int k = 0; k < m; ++k)
			{
				if (k > j) break;
				if (j - k > (m - 1) * i) continue;
				OpPlusEqu(arr[i][j], arr[i - 1][j - k]);
			}
		}
	}
	cout << ToString(arr[n - 1][(m - 1) * (n / 2)]);
	return 0;
}
