...
int main()
{
	int n, a, b, c, d, cnt = 1;
	scanf("%d", &n);
	kektor<pair<int, int> > arr(n);
	for (int i = 0; i < n; ++i)
	{
		scanf("%d:%d %d:%d", &a, &b, &c, &d);
		arr[i] = make_pair(c * 60 + d, a * 60 + b);
	}
	sort(arr.begin(), arr.end());
	AVL strs = new node(arr[0].first, 1);
	for (int i = 1; i < n; ++i)
	{
		AVL p = finda(strs, arr[i].second, strs);
		if (p->key < arr[i].second)
		{
			--cnt;
			p->value--;
			if (p->value == 0) strs = remove(strs, p->key);
		}
		AVL q = finde(strs, arr[i].first);
		if (q) q->value++;
		else strs = insert(strs, arr[i].first, 1);
		++cnt;
	}
	printf("%d", cnt);
	return 0;
